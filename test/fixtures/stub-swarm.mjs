...
if (verb === 'status') {
    ...
    if (path) {
        ...
        emit({
            ...
            changePlans: [{ path: 'change-plans/p.md', level: 'clean', diagnostics: [] }],
            workspaceFindings: [],
        });
    }
} else if (verb === 'review') {
    ...
    emit({
        ...
        coverage: [{ id: 'AC-002', kind: 'uncovered', message: 'requirement AC-002 is in scope but has no coverage row (uncovered)' }],
    ...
    if (kind === 'checks') {
        emit({ level: 'clean', kind: 'checks', value: { version: '0.6.0', checks: [{ id: 'C001', name: 'unique-ids', severity: 'hard-error' }] } });
    } else if (kind === 'task' && ref) {
        emit({ level: 'clean', kind: 'task', value: { id: `TASK-${ref}`, source: 'SPEC-x', status: 'ready', scope: ['AC-001'], affectedAreas: ['src'], claimedChangedFiles: [] } });
    } else if (kind === 'spec' && ref) {
        emit({ level: 'clean', kind: 'spec', value: { frontmatter: { type: 'spec', id: ref, status: 'ready', format: null, sources: [] }, requirements: [{ id: 'AC-001', line: 5, verifyCommand: 'a test' }], sectionTitles: ['Requirements'], openQuestionsPresent: false } });
    } else if (kind === 'review' && ref) {
        emit({ level: 'clean', kind: 'review', value: { status: 'needs-human', sectionTitles: ['Requirement coverage'], coverageRows: [{ id: 'AC-001', result: 'Pass', evidence: 'pasted' }], verifyBlocks: [] } });
    } else {
        process.stdout.write(JSON.stringify({ error: 'Usage', message: `cannot show ${kind} ${ref ?? ''}` }));
        process.exit(2);
    }
} else {
